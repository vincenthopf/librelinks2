...
export default async function handler(req, res) {
  try {
    ...
    console.log(`Fetching locations data for handle: ${handle}, timeRange: ${timeRange}`);

    // Get the token from environment variables
    const dashboardToken =
      process.env.NEXT_PUBLIC_TINYBIRD_WEB_ANALYTICS_DASHBOARD_TOKEN ||
      process.env.LOCATION_ANALYTICS_TOKEN;
    ...
    try {
      // Make the request to Tinybird - use top_locations instead of analytics_locations
      const response = await axios.get(
        'https://api.us-east.tinybird.co/v0/pipes/top_locations.json',
        {
          params: {
      ...
      if (pipeError.response) {
        if (pipeError.response.status === 404) {
          console.warn(
            'The pipe "top_locations" does not exist in Tinybird. Returning empty data.'
          );
          return res.status(200).json({
            data: [],
            totalVisits: 0,
            warning: 'The pipe "top_locations" does not exist in your Tinybird account',
          });
        } else if (pipeError.response.status === 403) {
          console.warn(
            'The token provided does not have access to the top_locations pipe. Returning empty data.'
          );
          ...
          return res.status(200).json({
            data: [],
            totalVisits: 0,
            warning: 'Token does not have access to the top_locations pipe',
          });
        }
      }
