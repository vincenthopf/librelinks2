...
import { useElementDimensions } from '@/utils/resize-observer';
import { calculateScaling, getScalingStyles } from '@/utils/embed-scaling';
...
const RichMediaPreview = ({ link, isMobile = false }) => {
  ...
  const { ref: containerRef, dimensions } = useElementDimensions();
  
  // Calculate scaling when container size changes
  const scaling = dimensions.width ? calculateScaling({
  ...
  }, [link, scriptLoaded, isLoading, dimensions, scaling]);
  ...
  // Get scaling styles
  const embedStyles = scaling ? getScalingStyles(scaling) : {};
