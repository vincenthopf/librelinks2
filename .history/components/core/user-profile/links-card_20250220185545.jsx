...
const LinkCard = (props) => {
  ...
  return (
    <div className="w-full mb-4 transition-all duration-300">
      <div className="relative">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-medium flex-grow truncate">{props.title}</h2>
            {hasPreview && (
              <button
                onClick={(e) => {
                  e.preventDefault();
      ...
      <div 
        className={`preview-wrapper overflow-hidden transition-all duration-500 ease-in-out ${
          showPreview ? 'max-h-[600px] opacity-100 mt-2' : 'max-h-0 opacity-0'
        }`}
      >
        <div className={`preview-content transition-transform duration-500 ease-in-out ${
          showPreview ? 'translate-y-0' : 'translate-y-4'
        }`}>
