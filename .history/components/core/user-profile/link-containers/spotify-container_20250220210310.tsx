...
const SpotifyContainer: React.FC<SpotifyContainerProps> = ({
  ...
  useEffect(() => {
    if (!embedHtml) {
      ...
      const height = '680'; // Set fixed height for all types
      ...
      try {
        ...
        iframe.style.minHeight = '680px'; // Added explicit min-height
    ...
    <ErrorBoundary>
      ...
      >
        ...
        >
          ...
          {embedHtml && (
            <div
              className={`spotify-embed w-full h-[680px] ${isLoading ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}
