...
- [ ] Analyze update behavior
  - ✓ preview-mobile.jsx updates correctly
  - ✗ preview.jsx (iframe) not updating
  - ✗ preview-btn.jsx drawer not refreshing
  - Document current update flow

## Fix Desktop Preview (preview.jsx)

- [ ] Update iframe refresh mechanism
  - Add key prop to iframe for forced refresh
  - Ensure URL params trigger iframe reload
  - Add useEffect to handle padding changes
  - Test iframe update behavior

## Fix Preview Button (preview-btn.jsx)

- [ ] Enhance drawer updates
  - Add useEffect for padding change detection
  - Force PreviewMobile re-render on changes
  - Ensure drawer state preserves updates
  - Test drawer update behavior

## Implementation Approach

- [ ] Implement fixes following existing patterns
  - Use React Query's cache invalidation
  - Maintain existing signalIframe usage
  - Add necessary useEffect hooks
  - Keep consistent with other features

## Testing & Validation

- [ ] Test all preview panels
  - Verify iframe updates correctly
  - Check drawer preview updates
  - Test mobile preview updates
  - Ensure all panels stay in sync

## Implementation Steps:

1. Add key prop to iframe in preview.jsx:

```jsx
<iframe
  key={JSON.stringify(currentUser)} // Forces refresh on user changes
  seamless
  loading="lazy"
  title="preview"
  id="preview"
...
2. Update preview-btn.jsx drawer:

```jsx
const PreviewBtn = () => {
  const { data: currentUser } = useCurrentUser(); // Add this
...
3. Verify signalIframe triggers in PaddingSelector work:
...
const mutatePadding = useMutation(
  ...
  },
  {
    onSuccess: () => {
      ...
      signalIframe(); // Verify this works
    },
  }
);
```
...
1. Should we add any loading states during iframe refresh?
2. Do we need to handle any race conditions between updates?
3. Should we add error boundaries around the previews?
