...
const TextCardButtonSelector = () => {
  const { data: currentUser } = useCurrentUser();
  const [textCardButtonStyle, setTextCardButtonStyle] = useState(null);
  const buttonFromDB = currentUser?.textCardButtonStyle;

  const queryClient = useQueryClient();

  useEffect(() => {
    const storedButton = buttonFromDB || localStorage.getItem('text-card-button-style');
    if (storedButton) {
      setTextCardButtonStyle(storedButton);
    }
    getThemeStyles();
  }, [buttonFromDB]);
  ...
  const handleChangeBtn = async buttonCSS => {
    ...
    setTextCardButtonStyle(buttonCSS);
    localStorage.setItem('text-card-button-style', buttonCSS);
  };

  const getThemeStyles = css => {
    if (textCardButtonStyle === css) {
      return {
  ...
  return (
    <div className="max-w-[640px] mx-auto my-4">
      <h3 className="text-xl font-semibold">Text Card Buttons</h3>
...
export default TextCardButtonSelector;
